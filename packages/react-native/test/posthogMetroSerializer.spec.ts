...
describe('PostHog Metro serializer', () => {
  ...
  test('keeps custom serializers working when they do not call the bundle callback', async () => {
    const customSerializer: MetroSerializer = async () => ({
      code: `${createDebugIdSnippet('__POSTHOG_CHUNK_ID__')}\nconsole.log("bare-rn")`,
      map: JSON.stringify({ version: 3, sources: [], names: [], mappings: '' }),
    })
    const serializer = createPostHogMetroSerializer(customSerializer)
    ...
    const chunkId = determineDebugIdFromBundleSource(result.code)
    const map = JSON.parse(result.map) as { chunkId?: string }
    expect(chunkId).toMatch(UUID_PATTERN)
    expect(result.code).not.toContain('__POSTHOG_CHUNK_ID__')
    expect(map.chunkId).toBe(chunkId)
  })
})
